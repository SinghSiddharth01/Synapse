...
# ── W2 residuals: the tools that were still resolving machine-wide ──────────


def _window(wiring, session: str, shared_id: str, *, contributor: str = "sid",
            pinned=TS, mirror: bool = False) -> None:
    """One conversation's own binding, as a real `join` leaves it: the
    per-session file always, the compatibility mirror only for whichever
    window joined last."""
    paths = [wiring.state_dir / "bindings" / "claude-code" / f"{session}.json"]
    if mirror:
        paths.append(wiring.binding_file)
    for path in paths:
        write_binding(path,
                      SessionBinding(agent_session_id=session, shared_id=shared_id,
                                     contributor=contributor, agent="claude-code",
                                     transcript_path=f"/tmp/{session}.jsonl",
                                     pinned_at=pinned))


async def test_an_unknown_session_id_does_not_borrow_a_sibling_windows_binding(tmp_path):
    """An id matching no binding used to fall back to the machine's
    most-recently-pinned one and stamp the caller's real id onto it. A
    conversation that had never joined anything therefore read and wrote a
    teammate's Shared Session while believing itself correctly bound — and the
    substitution meant nothing downstream could tell it apart from a real
    member.

    `scope` is what separates the two cases the fallback conflated: a
    `machine`-scoped binding is a stand-in any conversation may speak under
    (the `serve_local.py` demo path, asserted in its own test above and
    unchanged), a `session`-scoped one is ONE conversation's, and
    contracts/binding.py already specifies the reader's duty against it —
    refuse on a mismatch.

    The no-egress assertion is the load-bearing one: `_NOT_JOINED` in the text
    would be satisfied by a tool that answered honestly AFTER reading the
    other session."""
    urls: list[str] = []
    wiring = _wire(tmp_path, _service(urls=urls))
    _window(wiring, "conv-a", "sh-a", mirror=True)

    text = str(await wiring.server.call_tool(
        "query", {"question": "timing?", "agent_session_id": "conv-stranger"}))

    assert _NOT_JOINED in text
    assert urls == [], "an unbound conversation must not reach another session at all"


async def test_end_session_closes_the_callers_session_not_the_latest_pinned(tmp_path):
    """The one tool whose mistake cannot be undone. W2 moved every other
    conversation-acting tool off the machine-wide pick and missed this one, so
    with two windows joined to two Shared Sessions, ending from window A
    closed B's — destroying a team's memory, for everyone, on a call the user
    made about a different session entirely."""
    urls: list[str] = []
    wiring = _wire(tmp_path, _service(urls=urls))
    _window(wiring, "conv-a", "sh-a")
    # Window B joined later, so it owns the mirror and wins `_resolve_binding`.
    _window(wiring, "conv-b", "sh-b", pinned=datetime(2026, 8, 7, tzinfo=timezone.utc),
            mirror=True)

    text = str(await wiring.server.call_tool("end_session", {"agent_session_id": "conv-a"}))

    assert "POST http://svc/v1/sessions/sh-a/end" in urls
    assert not any("sh-b" in url for url in urls), (
        f"ended the wrong session — B's memory is gone: {urls}")
    assert "sh-a" in text


async def test_the_identity_is_the_callers_own_binding_not_a_sibling_windows(tmp_path):
    """`_identity` read `resolve_binding()`, which on a two-window machine
    answers with whichever window joined LAST. So a create/join from window A
    was performed as window B's contributor — reaching the one field that
    decides who OWNS a session and whom its creator-only end gate will later
    admit.

    The sibling is deliberately pinned LATER and given a different contributor,
    because equal names are how this stayed invisible: every other test in this
    file uses one name for both windows."""
    bodies: dict = {}
    wiring = _wire(tmp_path, _service(bodies=bodies), contributor="configured-not-this")
    _window(wiring, "conv-a", "sh-a", contributor="aditya")
    _window(wiring, "conv-b", "sh-b", contributor="bob",
            pinned=datetime(2026, 8, 7, tzinfo=timezone.utc), mirror=True)
    _transcript(wiring.projects, wiring.repo, "conv-a")

    await wiring.server.call_tool(
        "join_session", {"shared_id": "sh-team", "agent_session_id": "conv-a"})

    assert bodies["/v1/sessions/sh-team/members"] == {"contributor": "aditya"}


def test_counting_bound_conversations_dedupes_the_mirror_and_skips_machine_scope(tmp_path):
    """What decides whether the briefing may name a session at all, so its two
    edge cases are both failure modes with teeth.

    Counting FILES would make every ordinary single-window machine ambiguous:
    since W2 one join writes `bindings/claude-code/<session>.json` AND
    refreshes the mirror `bindings/claude-code.json`, so one conversation is
    two files. Every user would lose their briefing.

    Counting the `machine`-scoped stand-in would break the documented
    `serve_local.py` demo path in the same way: its one binding means "any
    conversation here speaks for this", so it is not one conversation among
    several and cannot be the ambiguity being measured."""
    wiring = _wire(tmp_path, _service())

    _window(wiring, "conv-a", "sh-a", mirror=True)
    assert cli._count_bound_conversations(wiring.state_dir) == 1, "two files, one conversation"

    _window(wiring, "conv-b", "sh-b", pinned=datetime(2026, 8, 7, tzinfo=timezone.utc))
    assert cli._count_bound_conversations(wiring.state_dir) == 2
    ...
    write_binding(wiring.state_dir / "bindings" / "codex.json",
                  SessionBinding(agent_session_id="as-sid", shared_id="local-dev",
                                 ...
                                 transcript_path="/tmp/scratch.jsonl", pinned_at=TS,
                                 scope="machine"))
    assert cli._count_bound_conversations(wiring.state_dir) == 2, "a stand-in is not a window"
