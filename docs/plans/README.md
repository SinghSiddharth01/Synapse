...
- `docs/adr/` — the four decisions worth a record: the local orchestrator, semantic merge with tombstones, the distiller compressing rather than judging, and **the log being append-only with state derived by folding it (`0004`, which supersedes `0002`'s mechanism)**.
...
- `docs/2026-08-05-service-implementation-report.md` — the same for the service track. **Plan C Task C.2 is superseded by it**, and C.4's merge input changed: without retrieved candidates in the prompt, two Contributors' findings only merge if they arrive in the same batch. Read it before executing Plan C.
...
4. **Findings are durable the moment they are produced**, before any send is attempted, and retained after sending. Distillation is the most expensive step in the system and is unrepeatable — the worker never re-reads a transcript position. Retention also makes a service restart recoverable by `resync`, which is safe because the service log is **append-only** and a replayed append is inert — see `adr/0004`.
5. **A merged Finding is a new record; originals become tombstones, never deletions.** Never discard-one (the second half of a pooled insight is the point) and never rewrite in place (the id would point at text its author never wrote). ⟨0004⟩ The *mechanism* is now derivation rather than a written field: a finding leaves the view because a later `Merged` entry names it as a source, not because anything was written onto it. `Finding.merged_into` is not written by the service — read visibility from the folded view.
