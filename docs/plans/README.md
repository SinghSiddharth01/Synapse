...
| [E6 — Debug dashboards](./exec/2026-08-05-e6-debug-dashboards.md) | instrumented `/debug` pages: `synapse-worker` (`--debug-port`, stdlib `http.server`, tagged `tick`/`triage`/`render`/`llm`/`push`/`error` feed) and `synapse-service` (Starlette-mounted, log tail + tagged `llm`/`query`/`synthesis` feed); `RecordingProvider` instruments any `ModelProvider` transparently for both | E5 (wraps the merged store, synthesis and retrieval on `main`) | **Built on `exec/e6`, task-by-task TDD — full suite green (549 passed: the 526 floor + 23 new).** Adversarially reviewed 2026-08-05, 4 findings, all fixed on `exec/e6`: the worker `/debug` read-only claim was pinned by test name only (test now compares repeated GETs against a pre-request snapshot); `--debug-port`'s default-on bind could raise an unhandled `OSError` and abort `synapse-worker run` (now caught, dashboard disabled, command continues); the service `/debug` had no off switch and always wrapped the provider (now `debug=False` / `--no-debug` / `SYNAPSE_SERVICE_DEBUG=0`); and both pages' expanded llm entries collapsed on the next 1s poll (now persisted via a `Set` of expanded keys, verified by actually running each page's `<script>` under Node). Read-only and provider-untouching, pinned by test on both sides. Full suite green at 565 (549 + 16 fixer tests) | Siddsing |
