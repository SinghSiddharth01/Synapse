...
Two adversarial review rounds surfaced real gaps in the code listing above; the shipped `synthesis.py` diverges from it in four ways, all adjudicated and none reverting the plan's own semantics (the "merge-of-one" behavior below is a restoration, not a new evolution — a mid-review deviation had inverted it and was reverted back to what's written here):

1. **Whole-verdict structural validation, not per-entry defense.** The code listing above indexes `merge["source_ids"]`/`merge["text"]`/`merge["type"]` and `c["a"]`/`c["b"]`/`c["description"]` directly. `AIC100Provider._satisfies_schema` only checks the TOP-level object's required keys and never inspects array items, so a `schema_valid=True` verdict can carry a merge/conflict entry missing a key — and did, in practice, crash `merge()` mid-application (`KeyError`), leaving tombstones written and `memory_version` un-bumped. The shipped code validates the entire verdict into a small pydantic model (`_SynthesisVerdicts` / `_MergeVerdict` / `_ConflictVerdict`) BEFORE any mutation: invalid → log, return `ctx` unchanged, exactly like the existing `schema_valid=False` path. No partial application, ever — a malformed entry anywhere invalidates the whole verdict, not just that entry.
2. **`CANDIDATE_WINDOW` never starves the current push.** `candidates = store.retrievable(shared_id)[-CANDIDATE_WINDOW:]` above is a pure recency slice of the whole Log that ignores `new_findings` entirely — a single push of more than `CANDIDATE_WINDOW` (20) findings silently starved the oldest of them, forever (they age out of the window the moment anything newer arrives). Shipped: candidates are the union of every finding new in THIS push plus up to `CANDIDATE_WINDOW` most-recent OTHERS. The bound still protects against Log growth (the fixed-cost property this task's docstring names); it now never protects against the current push. `api.py`'s `push_findings` passes the actual pushed `findings` list into `merge()` (not `[]`) so this union has something to work with — see the Task 4 amendment below.
3. **Conflicts resolve forward across EVERY round, not just within one.** The plan's three-line conflict loop (append if both ids are `known`) doesn't address what happens when a Conflict recorded in round N references a finding merged away in round N+1 — the reference dangles at a tombstone, which ADR 0002 explicitly names tombstones as needing to survive for ("Conflicts must follow it forward"). Shipped: after applying each round's verdict, ALL of `ctx.conflicts` — old rounds' and this round's alike — is resolved forward through `merged_into` via `_resolve_forward`, then deduplicated on the resolved (not raw) ids, so a re-reported pair naming a now-tombstoned id collides with the already-resolved entry instead of double-counting.
4. **Merge-of-one is honored, matching the plan's own log line.** `test_unknown_ids_from_the_model_are_ignored_not_fatal` (Step 1 above) asserts a verdict naming one real id and one ghost id still mints a Synthesized Finding from the one valid source (`merged_from == ["f-005a-01"]`) — the code listing's own log message says "applying to the N valid source(s)", not "skipping because fewer than 2". A mid-review deviation required `len(sources) >= 2` and inverted the test to match, undisclosed; round-2 adjudication restored the plan's semantics exactly.

See `packages/service/src/synapse_service/synthesis.py` for the current implementation and `packages/service/tests/test_synthesis.py` for the tests pinning all four.
...
The code listing above computes `watermark`'s `by_type` from the unfiltered Log (`Counter(f.type.value for f in store.retrievable(sid))`) and never touches `new_since`/`conflicts`. Round-2 adjudication on watermark suppression, after two review rounds flagged the gap and a half-fix:

- **`by_type` and `conflicts` are CONTENT fields.** They describe what THIS asker would actually see, so both now run through the same all-attributions suppression rule as `/query` (invariant 3) — an asker whose own Agent Session produced every attribution on a Finding, or on both sides of a Conflict, does not see it counted. Without this, `/v1/sessions/{sid}/watermark` advertised "new team knowledge" built entirely from the asker's own findings that `/query` then correctly refused to return.
- **`version` and `new_since` are CHANGE fields and stay global**, unfiltered by suppression — they measure how much the Shared Memory moved, not whether that movement is visible to this particular asker. A mid-review deviation zeroed `new_since` whenever nothing was visible, chasing agreement with `by_type`; round-2 adjudication reverted that and made the split explicit instead: `new_since > 0` while `by_type == {}` is not a contradiction to paper over (it can legitimately happen — e.g. a version bump that was entirely the asker's own findings merging with each other) — it is the intended distinction between "the memory changed" and "here is what changed for you". A `conflicts` count follows the same content rule as `by_type`, counting a Conflict when at least one side is visible to the asker.

`by_type` is the field E4's Relay and briefing composer consume verbatim (this task's Interfaces block); this amendment is the interface those callers should build against. See `packages/service/src/synapse_service/api.py`'s `watermark` handler and `packages/service/tests/test_api.py` for both halves pinned.

Separately, `push_findings` now passes the actual pushed `findings` list into `synthesizer.merge(...)` instead of `[]` (`store.upsert` is idempotent, so `merge()`'s own upsert of the same list is a harmless no-op) — see the Task 2 amendment's CANDIDATE_WINDOW point, and adds a `synthesized: bool` field to the push response (true iff `memory_version` actually moved this round) so a producer can distinguish a completed merge from one that silently failed or was skipped.

Two more gaps closed, neither in the listing above: `create_session`, `add_member`, and `query` now return `422 {error}` on a missing required body field (only `push_findings` implemented this task's own documented `422` contract; the other three indexed `body["..."]` directly and raised an unhandled 500 — E4's Relay treats 5xx as retryable, so this turned a client bug into an infinite retry). And Plan C's own stated resync story ("a full resync of a machine's entire log after a service restart converges to the same state as the original stream", `docs/plans/2026-08-03-plan-c-service.md:55`) still has no corresponding endpoint or test anywhere in `packages/service` — flagged in review, not built in this pass, no adjudication authorized the scope; `synthesized` above is a partial mitigation (the producer can at least SEE a failed round) not a substitute for it.
...
A mid-review deviation aliased `extract_first_json_object` to `openai_compat`'s private `_parse_json_tolerantly`, coupling a standalone provider (whose whole docstring rationale is NOT being an `OpenAICompatibleProvider` subclass) to another provider's internals, and changing semantics from "first balanced object" to "outermost brace span" — a regression against the common Llama-3.1-8B shape of narrating around the JSON or appending a trailing example. Round-2 adjudication reverted that aliasing and restored a standalone `extract_first_json_object` in `aic100.py` — but **not** the hand-rolled depth-counting loop this task's own Step 3 listing shows above.

**⟨Correction, this pass⟩ The sentence that used to stand here was self-contradictory and is replaced.** It claimed the shipped code was "the plan's standalone `extract_first_json_object` exactly as written above ... (string-aware via `json.JSONDecoder.raw_decode` scanned at every `{`, not hand-rolled brace-counting)" — but the Step 3 listing above *is* hand-rolled brace-counting (a manual `depth` counter over the string), so "exactly as written above" and "not hand-rolled brace-counting" cannot both be true, and neither reading matches what shipped. **The Step 3 listing is superseded, not restored.** The shipped `extract_first_json_object` (`packages/providers/src/synapse_providers/aic100.py`) is a different implementation of the same idea: try a cheap whole-string parse first, then a fenced ` ```json ` block, then scan every `{` in the text with `json.JSONDecoder().raw_decode` — string-aware (a `}` inside a quoted value never closes the object early) and first-match-wins, which the listing's hand-rolled counter is not. See the parametrized regression table in `packages/providers/tests/test_aic100.py::test_json_extraction_finds_the_first_object_around_narration_and_code`.

Also added, not in the listing above: `finish_reason == "length"` is now logged distinctly on the retry path. `max_tokens=800` (the constructor default above, unchanged) is tight against `SYNTH_SYSTEM`'s under-500-word rewrite plus JSON scaffolding, and a truncated response previously took the identical code path as unparseable garbage — extract fails, retry, extract fails again, honest `schema_valid=False` — with no signal anywhere distinguishing "cut off" from "the model was wrong". The default itself is left untouched (it's part of this task's documented constructor contract); an operator watching logs now has the information needed to raise it deliberately.

Two more shipped deviations, undocumented until now (E3 residual; verified directly against `aic100.py` as shipped, not the Step 3 listing):

- **`_satisfies_schema` gates `schema_valid=True`.** The Step 3 listing treats any `parsed is not None` as success; the shipped `complete()` additionally requires `_satisfies_schema(parsed, response_schema)` before returning `schema_valid=True` — a structural check (only ONE of the schema's declared `required` keys need be present, matching how `synapse_service.synthesis` validates the verdict through its `_SynthesisVerdicts` model with every field defaulted, so an 8B dropping one key doesn't cost the rest of an otherwise-usable round) that catches JSON which parsed cleanly but has nothing to do with the requested schema — the same false-confidence failure this whole module exists to catch one level up ("a 200 proves nothing was enforced").
- **The retry is a repair prompt at `temperature: 0.2`, not an identical resend.** The Step 3 listing's loop reissues the exact same `prompt` at the same implicit `temperature: 0.0` on both attempts of its `for _attempt in range(2)`; since `temperature: 0.0` is a deterministic decode, a second identical request is a guaranteed-identical second failure that only doubles consumption of the shared credit pool for nothing. The shipped retry instead rebuilds the prompt with the previous (truncated) response echoed back and an explicit "did not match the schema ... return the corrected JSON object" instruction, and moves `temperature` to `0.2` on attempt 2 so the resend is not a no-op.
