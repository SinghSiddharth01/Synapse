...
<style>
  ...
  :root[data-theme="light"] .stat.fold { box-shadow: none; }
  * { box-sizing: border-box; }
  ...
  .tool:focus-visible { outline: 2px solid var(--cyan); outline-offset: 2px; }
...
(function () {
  ...
  // headless test DOM (no documentElement, no localStorage) never notices.
  try {
    ...
    if (btn && btn.addEventListener) btn.addEventListener("click", flip);
    if (typeof document.addEventListener === "function") {
      document.addEventListener("keydown", function (ev) {
        var tag = ev.target && ev.target.tagName;
        if (tag === "INPUT" || tag === "TEXTAREA" || tag === "SELECT") return;
        if (ev.key === "t" || ev.key === "T") flip();
      });
    }
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .slide .sub { color: var(--ink-muted); font-size: 17px; margin: 0 0 28px; max-width: 80ch; }
  .hw {
    font-style: normal; font: 650 0.82em var(--mono);
    padding: 1px 9px; border-radius: 999px; border: 1px solid; white-space: nowrap;
  }
  .hw-npu { color: var(--copper); border-color: rgba(224, 154, 90, 0.45); background: rgba(224, 154, 90, 0.1); }
  .hw-ai100 { color: var(--cyan); border-color: rgba(20, 198, 203, 0.45); background: rgba(20, 198, 203, 0.1); }
  ...
  .net text { font: 650 16px var(--mono); letter-spacing: 0.05em; fill: var(--ink); }
  .net text.sub2 { font-weight: 500; font-size: 13.5px; letter-spacing: 0.02em; fill: var(--ink-muted); }
  .net text.sub3 { font-weight: 600; font-size: 13.5px; letter-spacing: 0.02em; fill: var(--copper); }
  .net text.core-label { fill: var(--cyan); letter-spacing: 0.14em; }
  .net text.chip-label { fill: var(--cyan); letter-spacing: 0.1em; font-size: 15px; }
  .net-caption { margin: 14px 0 0; text-align: center; color: var(--ink-muted); font-size: 15.5px; }
  ...
  .why-panel h3 { margin: 0 0 4px; font-size: 21px; font-weight: 650; letter-spacing: -0.015em; }
  .why-panel.bad h3 { color: var(--red-text); }
  .why-panel.good h3 { color: var(--green); }
  .why-panel .cap { margin: 0 0 12px; color: var(--ink-muted); font-size: 15px; }
  .why-panel svg { width: 100%; height: auto; display: block; }
  .why-panel .wire { fill: none; stroke: var(--ink-subtle); stroke-width: 2; opacity: 0.8; }
  .why-panel .wire.hot { stroke: var(--cyan); opacity: 0.95; stroke-width: 2.2; }
  .why-panel .wire.good { stroke: var(--green); opacity: 0.95; stroke-width: 2.2; }
  .why-panel .relay { fill: none; stroke: var(--amber); stroke-width: 2; stroke-dasharray: 6 7; opacity: 0.85; }
  .why-panel .aget { fill: var(--copper); }
  .why-panel .aname { font: 650 16px var(--mono); fill: var(--ink); }
  .why-panel .anote { font: 600 14.5px var(--sans); fill: var(--ink-muted); }
  .why-panel .anote.warn { fill: var(--amber); font-weight: 650; }
  .why-panel .deadx { stroke: var(--red-text); stroke-width: 4; stroke-linecap: round; }
  .why-panel .hub { fill: var(--cyan); }
  .why-panel .hub-halo { fill: none; stroke: var(--cyan); stroke-width: 1.5; opacity: 0.45; }
  .why-panel .hub-ring { fill: none; stroke: var(--cyan); stroke-width: 1.4; opacity: 0; }
  .why-panel .hubname { font: 650 14px var(--mono); fill: var(--cyan); letter-spacing: 0.08em; }
  .why-panel .pulse { fill: var(--cyan); opacity: 0; }
  .why-panel .tick { stroke: var(--green); stroke-width: 3.2; fill: none; stroke-linecap: round; stroke-linejoin: round; }
  .why-panel .relaydot { fill: var(--amber); opacity: 0; }
  .why-note {
    margin: 16px 0 0; padding: 20px 26px;
    background: linear-gradient(150deg, rgba(224, 154, 90, calc(var(--wash) + 0.05)), transparent 55%), var(--surface-1);
    border: 1px solid rgba(224, 154, 90, 0.4);
    border-radius: var(--radius);
    color: var(--ink-muted); font-size: 16.5px; line-height: 1.7; font-weight: 500;
  }
  .why-note .ex-tag {
    display: inline-block; margin: 0 0 10px;
    font: 650 12px var(--sans); letter-spacing: 0.12em; text-transform: uppercase;
    color: var(--copper);
    padding: 3px 12px; border-radius: 999px;
    border: 1px solid rgba(224, 154, 90, 0.45); background: rgba(224, 154, 90, 0.1);
  }
  .why-note h4 { margin: 0 0 8px; font-size: 20px; font-weight: 650; letter-spacing: -0.015em; color: var(--ink); }
  .why-note p { margin: 0; }
  ...
  .case {
    --case-c: var(--blue-text);
    --case-rgb: 110, 166, 255;
    display: flex; flex-direction: column; gap: 0;
    background: linear-gradient(160deg, rgba(var(--case-rgb), calc(var(--wash) - 0.02)), transparent 45%), var(--surface-1);
    border: 1px solid var(--hairline);
    border-top: 2px solid var(--case-c);
    border-radius: var(--radius);
    padding: 18px 20px;
    transition: border-color 140ms, transform 140ms, box-shadow 140ms;
  }
  .case:hover {
    border-color: rgba(var(--case-rgb), 0.55);
    transform: translateY(-3px);
    box-shadow: 0 0 42px -18px rgba(var(--case-rgb), 0.5);
  }
  .case.c-red    { --case-c: var(--red-text);    --case-rgb: 230, 43, 30; }
  .case.c-blue   { --case-c: var(--blue-text);   --case-rgb: 24, 104, 242; }
  .case.c-purple { --case-c: var(--purple-text); --case-rgb: 123, 66, 188; }
  .case.c-cyan   { --case-c: var(--cyan);        --case-rgb: 20, 198, 203; }
  .case.c-amber  { --case-c: var(--amber);       --case-rgb: 224, 154, 90; }
  .case.c-green  { --case-c: var(--green);       --case-rgb: 0, 202, 142; }
  .case-head { display: flex; gap: 13px; align-items: center; margin-bottom: 10px; }
  .case .cic {
    flex-shrink: 0; width: 42px; height: 42px; border-radius: 11px;
    display: flex; align-items: center; justify-content: center;
    background: rgba(var(--case-rgb), calc(var(--wash) + 0.04));
    border: 1px solid rgba(var(--case-rgb), 0.4);
    color: var(--case-c);
  }
  .case .cic svg { width: 22px; height: 22px; fill: none; stroke: currentColor; stroke-width: 1.7; stroke-linecap: round; stroke-linejoin: round; }
  .case b { display: block; font-size: 17px; font-weight: 650; color: var(--ink); letter-spacing: -0.012em; }
  .case .gist { display: block; color: var(--ink-muted); font-size: 13.5px; line-height: 1.45; font-weight: 500; margin-top: 2px; }
  .case .scen {
    margin: auto 0 0; padding: 11px 14px;
    background: var(--canvas);
    border-left: 2px solid var(--case-c);
    border-radius: 7px;
    color: var(--ink-muted); font-size: 14px; line-height: 1.6; font-weight: 500;
  }
  .case .scen b { display: inline; font-size: inherit; color: var(--ink); font-weight: 650; }
  ...
  .gh-row { margin-top: 16px; display: flex; justify-content: center; }
  .gh-card {
  ...
  .gh-card:hover { border-color: rgba(20, 198, 203, 0.5); transform: translateY(-2px); }
  .gh-card svg { width: 30px; height: 30px; fill: currentColor; flex-shrink: 0; }
  .gh-card b { display: block; font-size: 16.5px; font-weight: 650; }
  .gh-card span { display: block; font: 13.5px var(--mono); color: var(--ink-subtle); margin-top: 3px; }
  ...
  body[data-mode="slides"] .topbar.peek, body[data-mode="slides"] .topbar:hover { transform: translateY(0); }
  ...
  body[data-mode="slides"] .slide { display: none; width: 100%; max-height: 100%; overflow-y: auto; margin: 0; padding: 8px 6px; scrollbar-width: thin; }
  body[data-mode="slides"] .slide.on { display: block; }
  body[data-mode="slides"] .footer, body[data-mode="slides"] .scroll-only { display: none; }
  body[data-mode="slides"] .hero .kicker { font-size: 14px; margin-bottom: 16px; }
  ...
  body[data-mode="slides"] .case { padding: 22px 26px; }
  body[data-mode="slides"] .band-card { min-height: 205px; display: flex; flex-direction: column; justify-content: center; }
  body[data-mode="slides"] .case .scen { line-height: 1.66; }
  body[data-mode="slides"] .band { gap: 18px; }
  body[data-mode="slides"] .band-card { padding: 24px 26px; }
  ...
  body[data-mode="slides"] .band-card .num { font-size: 38px; }
  body[data-mode="slides"] .band-card.hero-stat .num { font-size: clamp(56px, 5vw, 84px); }
  body[data-mode="slides"] .band-card .what { font-size: 16px; line-height: 1.64; }
  body[data-mode="slides"] .band-card.hero-stat .what { font-size: 17.5px; }
  body[data-mode="slides"] .run { display: none; }
  body[data-mode="slides"] .gh-row { margin-top: 22px; }
  #hud {
    display: none;
...
<main id="deck">
  ...
  <section class="slide s-cases" data-reveal id="s-cases">
    ...
    <p class="sub">The same memory, six shapes of team. Each card is a moment where the memory pays for itself.</p>
    <div class="cases">
      <div class="case c-red">
        <div class="case-head"><span class="cic"><svg viewBox="0 0 24 24"><circle cx="12" cy="13" r="4.5"/><path d="M12 8.5V6.2M9 7l1.4 1.7M15 7l-1.4 1.7M4.5 13h3M16.5 13h3M6 18l2.1-1.8M18 18l-2.1-1.8M6 8.5l2.2 1.5M18 8.5l-2.2 1.5"/></svg></span><div><b>Debugging together</b><span class="gist">One teammate's dead end is everyone's dead end.</span></div></div>
        <p class="scen"><b>Sid</b> rules out a DMA race at 2pm. At 4pm <b>Akhil's</b> agent proposes the same theory, asks the memory first, and gets Sid's dead end back before opening a single file.</p>
      </div>
      <div class="case c-blue">
        <div class="case-head"><span class="cic"><svg viewBox="0 0 24 24"><circle cx="6.5" cy="6" r="2.2"/><circle cx="6.5" cy="18" r="2.2"/><circle cx="17.5" cy="9.5" r="2.2"/><path d="M6.5 8.2v7.6M6.5 12c0-2.6 4.4-2.2 8.8-2.4"/></svg></span><div><b>Feature work</b><span class="gist">Parallel building without parallel re-discovery.</span></div></div>
        <p class="scen"><b>Aditya</b> builds the parser while <b>Akhil</b> builds the CLI. The flag-name decision Aditya makes lands in the memory; Akhil's agent uses the same name without being told.</p>
      </div>
      <div class="case c-purple">
        <div class="case-head"><span class="cic"><svg viewBox="0 0 24 24"><path d="M12 3.5a5.7 5.7 0 0 0-3.2 10.4c.7.5 1.2 1.6 1.2 2.6h4c0-1 .5-2.1 1.2-2.6A5.7 5.7 0 0 0 12 3.5z"/><path d="M10 19.2h4M10.7 21.2h2.6"/></svg></span><div><b>Design and brainstorming</b><span class="gist">Decisions propagate the moment they're made.</span></div></div>
        <p class="scen">The team debates cache eviction in the morning. By afternoon every agent answers "what did we decide about eviction" with the decision, and with who decided it.</p>
      </div>
      <div class="case c-cyan">
        <div class="case-head"><span class="cic"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="1.8"/><path d="M8.5 15.5a5 5 0 0 1 0-7M15.5 8.5a5 5 0 0 1 0 7M6 18a8 8 0 0 1 0-12M18 6a8 8 0 0 1 0 12"/></svg></span><div><b>Status sharing</b><span class="gist">Your agent already knows what the team did today.</span></div></div>
        <p class="scen">"What happened with the flaky auth test?" is answered by the agent from the memory, with attribution, instead of by scrolling a chat thread for ten minutes.</p>
      </div>
      <div class="case c-amber">
        <div class="case-head"><span class="cic"><svg viewBox="0 0 24 24"><path d="M9.5 3.5h5M10.5 3.5v5.2L5.6 17a2.6 2.6 0 0 0 2.3 3.9h8.2A2.6 2.6 0 0 0 18.4 17l-4.9-8.3V3.5"/><path d="M8 14.5h8"/></svg></span><div><b>Lab and dev</b><span class="gist">On-target context meets code context in one memory.</span></div></div>
        <p class="scen"><b>Akhil</b> captures a crash signature on the bench rig; <b>Sid's</b> code-side stack trace lands the same hour. The join between them happens at query time, in one memory.</p>
      </div>
      <div class="case c-green">
        <div class="case-head"><span class="cic"><svg viewBox="0 0 24 24"><path d="M4 9h13M14 5.5L17.5 9 14 12.5"/><path d="M20 15H7M10 11.5L6.5 15l3.5 3.5"/></svg></span><div><b>Asymmetric teammates</b><span class="gist">MATLAB author and C++ porter, no manual relay.</span></div></div>
        <p class="scen">The MATLAB author fixes an off-by-one in the filter. The C++ porter's agent flags the same loop in the port before the bug ships twice.</p>
      </div>
    </div>
  </section>

  <section class="slide s-proof" data-reveal id="s-proof">
    <span class="slide-eyebrow">Proof</span>
    ...
    <div class="run scroll-only">
      ...
      <div class="run-right">
        <div class="providers">
          ...
          <div class="prov-row live"><span>NPU · GenieX · Qwen3-4B</span><span class="st">live now</span></div>
          <div class="prov-row live"><span>Cloud AI 100 · Llama-3.3-70B</span><span class="st">live now</span></div>
          <div class="prov-row live"><span>Anthropic API · Claude Haiku</span><span class="st">live now</span></div>
...
(function () {
  ...
  function go(n, instant) {
    if (!slideMode) return;
    // Wrap around: advancing past the last slide returns to the first, and
    // backing up from the first lands on the last. A deck that dead-ends on
    // its final slide re-renders it forever; a loop always has a way home.
    var len = slides.length;
    var next = ((n % len) + len) % len;
    if (next === cur && !instant && slides[cur].classList.contains("on")) return;
    cur = next;
  ...
  if (topbar) {
    document.addEventListener("mousemove", function (ev) {
      if (!slideMode) return;
      if (ev.clientY < 64) topbar.classList.add("peek");
      else if (!topbar.matches(":hover")) topbar.classList.remove("peek");
    });
  }
